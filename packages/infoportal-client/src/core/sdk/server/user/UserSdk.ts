...
export class UserSdk {
  ...
  readonly update = ({workspaceId, user}: {workspaceId: Ip.WorkspaceId; user: Partial<User>}) => {
    return this.client.post<User>(`${workspaceId}/user/me`, {body: user})
  }

  readonly avatarUrl = ({email}: {email: Ip.User.Email}) => {
    return `${this.client.baseUrl}/user/avatar/${email}`
  }

  readonly search = ({workspaceId}: {workspaceId: Ip.WorkspaceId}) => {
    return this.client.get<any[]>(`${workspaceId}/user`).then(res => res.map(User.map))
  }

  readonly fetchJobs = ({workspaceId}: {workspaceId: Ip.WorkspaceId}) => {
